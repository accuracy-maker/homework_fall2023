...
class PGAgent(nn.Module):
    ...
    def update(
        ...
        # beyond this point.
        # Flattening the lists of arrays
    ...
    def _estimate_advantage(
        ...
        if self.critic is None:
            ...
            # if no baseline, the advantages will be Q-values
            advantages = q_values
            
        else:
            # TODO: run the critic and use it as a baseline
            values = values = ptu.to_numpy(
        ...
        if self.normalize_advantages:
            gae_mean = np.mean(advantages)
            gae_std = np.std(advantages) + 1e-8
            advantages = (advantages - gae_mean) / gae_std
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        for i in range(len(rewards)):
            rewards_sum += self.gamma ** i * rewards[i]
        discounted_rewards = np.array([rewards_sum] * len(rewards))
